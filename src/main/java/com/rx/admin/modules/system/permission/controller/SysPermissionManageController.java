...
@RestController
@ApiVersion(1)
@RequestMapping("/sys/permission-manage")
@SaCheckRole("admin")
...
public class SysPermissionManageController {
    ...
    @SaCheckPermission("system:permission:manage")
    public Result<?> addUserMenus(@PathVariable Long userId, @RequestBody @Valid UserMenuDTO dto) {
    ...
    @SaCheckPermission("system:permission:manage")
    public Result<?> removeUserMenus(@PathVariable Long userId, @RequestBody @Valid UserMenuDTO dto) {
        List<Long> menuIds = dto.getMenuIds();
        if (menuIds == null || menuIds.isEmpty()) {
            return Result.fail("菜单ID不能为空");
        }
    ...
    @SaCheckPermission("system:permission:manage")
    public Result<?> setUserMenus(@PathVariable Long userId, @RequestBody @Valid UserMenuDTO dto) {
