package com.rx.admin.modules.monitor.health.service;
...
public class HealthService {
    ...
    public Map<String, Object> getSystemHealth() {
        ...
        // getSystemLoadAverage() 在 Windows 上返回 -1（不可用），此时用 0 避免显示负数
        double cpuUsage = loadAvg < 0 ? 0 : Math.round(loadAvg / Runtime.getRuntime().availableProcessors() * 1000.0) / 10.0;
        cpu.put("usage", cpuUsage);
        cpu.put("loadAverage", loadAvg < 0 ? 0 : Math.round(loadAvg * 100.0) / 100.0);
        data.put("cpu", cpu);
