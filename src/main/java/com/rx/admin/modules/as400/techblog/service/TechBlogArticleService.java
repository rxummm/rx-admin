...
import com.rx.admin.modules.as400.techblog.mapper.TechBlogArticleMapper;
...
public class TechBlogArticleService extends ServiceImpl<TechBlogArticleMapper, TechBlogArticle> implements ITechBlogArticleService {
    ...
    public TechBlogArticleService(DashboardCache dashboardCache, ApplicationEventPublisher eventPublisher) {
            ...
            for (Element link : pageLinks) {
                ...
                if (href.contains("/page/")) {
                    try {
                        ...
                        log.warn("解析分页页码失败: {}", e.getMessage());
                    }
                }
            }
            ...
            try {
                ...
                if (attempt < maxRetries) {
                    try { Thread.sleep(2000L * attempt); } catch (InterruptedException ie) {
                        log.warn("重试延迟被中断: {}", ie.getMessage());
        ...
        for (int year = currentYear; year >= 2021; year--) {
            for (int month = 12; month >= 1; month--) {
                ...
                try {
                    ...
                    log.warn("该月可能没有文章，正常跳过: {}", e.getMessage());
                }
            }
        }
        return totalSaved;
        ...
        try {
            ...
            for (DateTimeFormatter fmt : formatters) {
                try {
                    ...
                    log.warn("日期解析格式不匹配, 尝试下一个: {}", e.getMessage());
                }
            }
        } catch (Exception e) {
            log.warn("日期解析失败: {}", raw);
    ...
    private void setProgress(String source, int value) {
        ...
        // 抓取完成时标记仪表盘缓存为 dirty 并推送 SSE 立即刷新
        if (value >= 100 && dashboardCache != null) {
            dashboardCache.markTechblogDirty();
            eventPublisher.publishEvent(new DashboardChangeEvent(DashboardChangeEvent.SECTION_TECHBLOG));
        }
    }
}