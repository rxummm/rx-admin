<template>
  <div class="page-container">
    <div class="search-bar">
      ...
      <el-input
        v-model="keyword"
        :placeholder="$t('system.ipRule.searchPlaceholder')"
        style="width: 180px"
      ...
      <el-select
        v-model="ruleType"
        :placeholder="$t('system.ipRule.ruleType')"
        style="width: 120px"
        clearable
        @change="fetchData"
      >
    ...
    <div class="table-container">
      <el-table :data="tableData" v-loading="loading" border stripe>
        ...
        <el-table-column prop="ruleType" :label="$t('system.ipRule.ruleType')" width="100">
          <template #default="{ row }">
            <el-tag :type="row.ruleType === 'BLACK' ? 'danger' : 'success'" size="small">
              {{
                row.ruleType === 'BLACK'
                  ? $t('system.ipRule.ruleTypeOptions.black')
                  : $t('system.ipRule.ruleTypeOptions.white')
              }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column
          prop="description"
          :label="$t('system.ipRule.description')"
          min-width="200"
    ...
    <el-dialog
      ...
      :title="isEdit ? $t('system.ipRule.editRule') : $t('system.ipRule.addRuleTitle')"
      width="500px"
    >
...
const modeTip = computed(() => {
  return filterMode.value === 'BLACK'
    ? t('system.ipRule.filterBlack')
    : filterMode.value === 'WHITE'
      ? t('system.ipRule.filterWhite')
      : t('system.ipRule.filterOff')
})

const loadMode = async () => {
  try {
    await setIpRuleModeApi(filterMode.value)
    ElMessage.success(t('system.ipRule.switchModeSuccess'))
  } catch {
    /* */
  }
}
...
const fetchData = async () => {
  loading.value = true
  try {
    const res = await getIpRulePageApi({
      page: page.value,
      size: size.value,
...
const handleAdd = () => {
  isEdit.value = false
  form.value = { status: 1, ruleType: 'BLACK' }
...
const handleDelete = async (row) => {
  await deleteIpRuleApi(row.id)
  ElMessage.success(t('system.ipRule.deleteSuccess'))
  fetchData()
}
const handleSubmit = async () => {
  if (!form.value.ipAddress) {
    ElMessage.warning(t('system.ipRule.pleaseInputIp'))
    return
  }
  isEdit.value ? await updateIpRuleApi(form.value) : await addIpRuleApi(form.value)
  ElMessage.success(isEdit.value ? t('system.ipRule.modifySuccess') : t('system.ipRule.addSuccess'))
...
;(async () => {
  try {
    const res = await getIpRuleModeApi()
    filterMode.value = res.data?.mode || 'OFF'
  } catch {
    /* */
  }
  fetchData()
