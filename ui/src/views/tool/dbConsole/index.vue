    ...
    <el-row :gutter="16">
          ...
          <div style="max-height: 500px; overflow-y: auto">
            <div
              ...
              :style="{
                ...
                cursor: 'pointer',
                borderRadius: '4px',
                background: selectedTable === t.name ? 'var(--bg-active, #fef9ee)' : '',
                marginBottom: '2px'
              }"
              @click="selectTable(t.name)"
            >
              <span style="font-size: 13px">{{ t.name }}</span>
              <span v-if="t.comment" style="color: var(--text-secondary, #909399); font-size: 11px; margin-left: 4px">{{
                t.comment
              }}</span>
            </div>
          </div>
      ...
      <el-col :span="18">
        <el-card shadow="hover">
          ...
          <span v-if="execResult" style="margin-left: 12px; color: var(--text-secondary, #909399)"
            >{{ execResult.rowCount ?? execResult.affected }} 条 | {{ execResult.elapsed }}ms</span
          >
          <span v-if="execResult?.truncated" style="color: var(--color-warning, #e6a23c); margin-left: 8px"
            >结果已截断（最多1000行）</span
          >

          <el-table
            v-if="execResult?.type === 'query' && execResult.columns"
          ...
          >
            <el-table-column
              v-for="col in execResult.columns"
              :key="col"
              :prop="col"
              :label="col"
              min-width="120"
          ...
          <div
            v-if="execError"
            style="color: var(--color-danger, #f56c6c); margin-top: 8px; font-family: monospace; white-space: pre-wrap"
          >
            {{ execError }}
          </div>
        </el-card>
      ...
      <el-row :gutter="16" v-if="poolStatus">
        <el-col :span="4"
          ><div class="stat-item">
            ...
            ><span class="stat-value" style="color: var(--color-success, #67c23a)">{{
        ...
        <el-col :span="4"
          ><div class="stat-item">
            ...
            ><span class="stat-value" style="color: var(--text-secondary, #909399)">{{
        ...
        <el-col :span="4"
          ><div class="stat-item">
            <span class="stat-label">总数</span><span class="stat-value">{{ poolStatus.totalConnections }}</span>
          </div></el-col
        >
        <el-col :span="4"
          ><div class="stat-item">
            ...
            ><span
              class="stat-value"
              :style="{ color: poolStatus.threadsAwaitingConnection > 0 ? 'var(--color-danger,#f56c6c)' : '' }"
              >{{ poolStatus.threadsAwaitingConnection }}</span
            >
          </div></el-col
        >
        <el-col :span="4"
          ><div class="stat-item">
            <span class="stat-label">最大连接</span><span class="stat-value">{{ poolStatus.maximumPoolSize }}</span>
          </div></el-col
        >
      </el-row>
...
async function executeSql() {
  ...
  execError.value = ''
  try {
    const res = await executeSqlApi({ sql: sql.value })
    execResult.value = res.data
  } catch (e) {
    execError.value = e?.response?.data?.msg || e.message || '执行失败'
  } finally {
...
async function selectTable(name) {
  ...
  sql.value = `SELECT * FROM ${name} LIMIT 10`
  try {
    const res = await getTableColumnsApi(name)
    tableColumns.value = res.data
  } catch {}
}
async function fetchPoolStatus() {
  try {
    const res = await getPoolStatusApi()
    poolStatus.value = res.data
  } catch {}
}
