  ...
  <div class="page-container">
    ...
    <div v-if="step === 0" style="max-width: 600px; margin: 24px auto; text-align: center">
      ...
      >
        ...
        <template #tip
          ><div style="margin-top: 8px; color: var(--text-secondary, #909399); font-size: 12px">
            支持 .xlsx .xls .csv 格式
          </div></template
        >
      </el-upload>
    ...
    <div v-if="step === 1" style="flex: 1; min-height: 0; display: flex; flex-direction: column">
      <div style="margin-bottom: 12px; display: flex; gap: 16px; align-items: center">
        ...
        <el-tag v-if="analyzeResult.errorRows?.length" type="danger"
          >错误行: {{ analyzeResult.errorRows.length }}</el-tag
        >
      ...
      <el-table :data="analyzeResult.rows || []" border stripe max-height="400" size="small">
        ...
        <el-table-column
          v-for="(col, i) in analyzeResult.columns || []"
          :key="i"
          :prop="String(i)"
          :label="col"
          min-width="120"
    ...
    <div v-if="step === 2" style="max-width: 500px; margin: 24px auto">
      ...
      <div style="text-align: center; margin-top: 16px; color: var(--text-secondary, #909399)">
        {{ importProgressText }}
      </div>
    ...
    <div style="display: flex; justify-content: center; gap: 12px; margin-top: 16px">
      ...
      <el-button v-if="step === 0" type="primary" @click="doAnalyze" :disabled="!canAnalyze" :loading="analyzing"
        >下一步</el-button
      >
