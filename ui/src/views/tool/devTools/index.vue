  ...
  <div class="devtools-container">
    ...
    <div class="devtools-content">
      ...
      <div v-if="activeTool === 'json'" class="tool-panel">
        <h3 class="tool-title">
          {{ $t('tool.devTools.tools.json.name')
          }}<span class="tool-desc"> — {{ $t('tool.devTools.tools.json.desc') }}</span>
        </h3>
        <el-input
          v-model="jsonInput"
          type="textarea"
      ...
      <div v-if="activeTool === 'jsoneditor'" class="tool-panel">
        <h3 class="tool-title">
          {{ $t('tool.devTools.tools.jsoneditor.name')
          }}<span class="tool-desc"> — {{ $t('tool.devTools.tools.jsoneditor.desc') }}</span>
        </h3>
        <div class="tool-actions">
          ...
          <el-tag v-if="jsonEditValid !== null" :type="jsonEditValid ? 'success' : 'danger'" size="small">{{
            jsonEditValid ? '✓ 有效JSON' : '✗ 格式错误'
          }}</el-tag>
        </div>
        <div style="display: flex; gap: 12px; margin-top: 12px; min-height: 300px">
          <el-input
            v-model="jsonEditInput"
            type="textarea"
            :rows="14"
            placeholder='{"name":"value"}'
            style="flex: 1; font-family: monospace; font-size: 13px"
            @input="jsonEditOnInput"
          />
          <div class="json-tree-panel">
            <div v-if="jsonEditTree" class="json-tree-title">结构预览</div>
            <div
              v-if="jsonEditTree"
              v-html="sanitizeHtml(jsonEditTree)"
              style="overflow: auto; font-family: monospace; font-size: 12px; line-height: 1.6"
            ></div>
            <div v-if="!jsonEditTree && jsonEditInput" style="color: #c0c4cc; text-align: center; padding-top: 40px">
              输入有效JSON后显示结构
            </div>
          </div>
        </div>
      ...
      <div v-if="activeTool === 'jsonconvert'" class="tool-panel">
        <h3 class="tool-title">
          {{ $t('tool.devTools.tools.jsonconvert.name')
          }}<span class="tool-desc"> — {{ $t('tool.devTools.tools.jsonconvert.desc') }}</span>
        </h3>
        <el-input
          v-model="jsonConvertInput"
          type="textarea"
          :rows="8"
      ...
      <div v-if="activeTool === 'base64'" class="tool-panel">
        <h3 class="tool-title">
          {{ $t('tool.devTools.tools.base64.name')
          }}<span class="tool-desc"> — {{ $t('tool.devTools.tools.base64.desc') }}</span>
        </h3>
        <el-input
          v-model="base64Input"
          type="textarea"
      ...
      <div v-if="activeTool === 'urlcodec'" class="tool-panel">
        <h3 class="tool-title">
          {{ $t('tool.devTools.tools.urlcodec.name')
          }}<span class="tool-desc"> — {{ $t('tool.devTools.tools.urlcodec.desc') }}</span>
        </h3>
        <el-input
          v-model="urlInput"
          type="textarea"
      ...
      <div v-if="activeTool === 'unicode'" class="tool-panel">
        <h3 class="tool-title">
          {{ $t('tool.devTools.tools.unicode.name')
          }}<span class="tool-desc"> — {{ $t('tool.devTools.tools.unicode.desc') }}</span>
        </h3>
        <el-input
          v-model="unicodeText"
          type="textarea"
          :rows="4"
          placeholder="输入中文或Unicode转义字符串（如 \u4f60\u597d）"
          style="font-family: monospace"
        />
        <div class="tool-actions">
      ...
      <div v-if="activeTool === 'htmlentity'" class="tool-panel">
        <h3 class="tool-title">
          {{ $t('tool.devTools.tools.htmlentity.name')
          }}<span class="tool-desc"> — {{ $t('tool.devTools.tools.htmlentity.desc') }}</span>
        </h3>
        <el-input
          v-model="htmlInput"
          type="textarea"
          :rows="4"
          placeholder="输入HTML文本或实体字符串"
          style="font-family: monospace"
        />
        <div class="tool-actions">
      ...
      <div v-if="activeTool === 'radix'" class="tool-panel">
        <h3 class="tool-title">
          {{ $t('tool.devTools.tools.radix.name')
          }}<span class="tool-desc"> — {{ $t('tool.devTools.tools.radix.desc') }}</span>
        </h3>
        <div class="radix-row">
        ...
        <div class="radix-results" v-if="radixResults">
          <div class="radix-item">
            <span>> 二进制：</span><code>{{ radixResults.bin }}</code>
          </div>
          <div class="radix-item">
            <span>> 八进制：</span><code>{{ radixResults.oct }}</code>
          </div>
          <div class="radix-item">
            <span>> 十进制：</span><code>{{ radixResults.dec }}</code>
          </div>
          <div class="radix-item">
            <span>> 十六进制：</span><code>{{ radixResults.hex }}</code>
          </div>
        </div>
      </div>
      ...
      <div v-if="activeTool === 'crypto'" class="tool-panel">
        <h3 class="tool-title">
          {{ $t('tool.devTools.tools.crypto.name')
          }}<span class="tool-desc"> — {{ $t('tool.devTools.tools.crypto.desc') }}</span>
        </h3>
      ...
      <div v-if="activeTool === 'jwt'" class="tool-panel">
        <h3 class="tool-title">
          {{ $t('tool.devTools.tools.jwt.name')
          }}<span class="tool-desc"> — {{ $t('tool.devTools.tools.jwt.desc') }}</span>
        </h3>
        <el-input
          v-model="jwtToken"
          type="textarea"
          :rows="2"
          placeholder="粘贴JWT Token，如 eyJhbGciOiJIUzI1NiJ9..."
          style="font-family: monospace"
          @input="parseJwt"
        />
      ...
      <div v-if="activeTool === 'http'" class="tool-panel">
        <h3 class="tool-title">
          {{ $t('tool.devTools.tools.http.name')
          }}<span class="tool-desc"> — {{ $t('tool.devTools.tools.http.desc') }}</span>
        </h3>
        <div class="http-row">
          ...
          <el-input
            v-model="httpUrl"
            placeholder="输入请求URL，如 https://api.example.com/data"
        ...
        <div v-if="httpResponse !== null" class="http-response-box">
          <div class="http-response-header">
            <span
              >状态:
              <el-tag :type="httpStatus >= 200 && httpStatus < 300 ? 'success' : 'danger'" size="small">{{
                httpStatus
              }}</el-tag></span
            >
            <span style="margin-left: 12px">耗时: {{ httpDuration }}ms</span>
            <el-button link type="primary" size="small" style="margin-left: auto" @click="copyOne(httpResponse)"
              >复制</el-button
            >
          </div>
      ...
      <div v-if="activeTool === 'ipquery'" class="tool-panel">
        <h3 class="tool-title">
          {{ $t('tool.devTools.tools.ipquery.name')
          }}<span class="tool-desc"> — {{ $t('tool.devTools.tools.ipquery.desc') }}</span>
        </h3>
        <div class="tool-actions">
          <el-input
            v-model="ipQueryAddr"
            placeholder="输入IP地址，留空查当前IP"
      ...
      <div v-if="activeTool === 'uuid'" class="tool-panel">
        <h3 class="tool-title">
          {{ $t('tool.devTools.tools.uuid.name')
          }}<span class="tool-desc"> — {{ $t('tool.devTools.tools.uuid.desc') }}</span>
        </h3>
        <div class="tool-actions">
      ...
      <div v-if="activeTool === 'passgen'" class="tool-panel">
        <h3 class="tool-title">
          {{ $t('tool.devTools.tools.passgen.name')
          }}<span class="tool-desc"> — {{ $t('tool.devTools.tools.passgen.desc') }}</span>
        </h3>
      ...
      <div v-if="activeTool === 'qrcode'" class="tool-panel">
        <h3 class="tool-title">
          {{ $t('tool.devTools.tools.qrcode.name')
          }}<span class="tool-desc"> — {{ $t('tool.devTools.tools.qrcode.desc') }}</span>
        </h3>
        <div class="tool-actions">
      ...
      <div v-if="activeTool === 'regex'" class="tool-panel">
        <h3 class="tool-title">
          {{ $t('tool.devTools.tools.regex.name')
          }}<span class="tool-desc"> — {{ $t('tool.devTools.tools.regex.desc') }}</span>
        </h3>
        ...
        <el-input
          ...
          style="width: 480px; margin-bottom: 8px; font-family: monospace"
        />
      ...
      <div v-if="activeTool === 'textstats'" class="tool-panel">
        <h3 class="tool-title">
          {{ $t('tool.devTools.tools.textstats.name')
          }}<span class="tool-desc"> — {{ $t('tool.devTools.tools.textstats.desc') }}</span>
        </h3>
        ...
        <div v-if="textStats" class="text-stats-result">
          <div class="stat-card">
            <span class="stat-val">{{ textStats.chars }}</span
            ><span class="stat-lbl">字符数(含空格)</span>
          </div>
          <div class="stat-card">
            <span class="stat-val">{{ textStats.charsNoSpace }}</span
            ><span class="stat-lbl">字符数(无空格)</span>
          </div>
          <div class="stat-card">
            <span class="stat-val">{{ textStats.words }}</span
            ><span class="stat-lbl">字数(中文)</span>
          </div>
          <div class="stat-card">
            <span class="stat-val">{{ textStats.lines }}</span
      ...
      <div v-if="activeTool === 'mdhtml'" class="tool-panel">
        <h3 class="tool-title">
          {{ $t('tool.devTools.tools.mdhtml.name')
          }}<span class="tool-desc"> — {{ $t('tool.devTools.tools.mdhtml.desc') }}</span>
        </h3>
        <div class="tool-actions">
      ...
      <div v-if="activeTool === 'emojipicker'" class="tool-panel">
        <h3 class="tool-title">
          {{ $t('tool.devTools.tools.emojipicker.name')
          }}<span class="tool-desc"> — {{ $t('tool.devTools.tools.emojipicker.desc') }}</span>
        </h3>
        <div class="emoji-search-row">
          <el-input
            ...
            style="width: 260px"
            clearable
            @clear="clearEmojiSearch"
          />
          <div class="emoji-cats">
            <span
              v-for="c in emojiCategories"
              :key="c.key"
              class="emoji-cat-chip"
              :class="{ active: emojiCat === c.key }"
              @click="emojiCat = c.key"
              >{{ c.label }}</span
            >
          </div>
        </div>
        <div class="emoji-grid" v-if="filteredEmojis.length">
          <span v-for="e in filteredEmojis" :key="e.c" class="emoji-item" :title="e.n" @click="copyEmoji(e.c)">{{
            e.c
          }}</span>
        </div>
      ...
      <div v-if="activeTool === 'timestamp'" class="tool-panel">
        <h3 class="tool-title">
          {{ $t('tool.devTools.tools.timestamp.name')
          }}<span class="tool-desc"> — {{ $t('tool.devTools.tools.timestamp.desc') }}</span>
        </h3>
      ...
      <div v-if="activeTool === 'datecalc'" class="tool-panel">
        <h3 class="tool-title">
          {{ $t('tool.devTools.tools.datecalc.name')
          }}<span class="tool-desc"> — {{ $t('tool.devTools.tools.datecalc.desc') }}</span>
        </h3>
      ...
      <div v-if="activeTool === 'timezone'" class="tool-panel">
        <h3 class="tool-title">
          {{ $t('tool.devTools.tools.timezone.name')
          }}<span class="tool-desc"> — {{ $t('tool.devTools.tools.timezone.desc') }}</span>
        </h3>
        <div class="datecalc-row" style="margin-bottom: 12px">
          <el-date-picker
            v-model="tzSourceTime"
            type="datetime"
            placeholder="选择时间"
            value-format="YYYY-MM-DD HH:mm:ss"
            style="width: 220px"
        ...
        <div
          v-if="tzResult"
          class="ts-result"
          style="background: #f5f7fa; padding: 12px 16px; border-radius: 6px; font-size: 15px"
        >
          {{ tzResult }}
        </div>
      </div>
      ...
      <div v-if="activeTool === 'cron'" class="tool-panel">
        <h3 class="tool-title">
          {{ $t('tool.devTools.tools.cron.name')
          }}<span class="tool-desc"> — {{ $t('tool.devTools.tools.cron.desc') }}</span>
        </h3>
      ...
      <div v-if="activeTool === 'codefmt'" class="tool-panel">
        <h3 class="tool-title">
          {{ $t('tool.devTools.tools.codefmt.name')
          }}<span class="tool-desc"> — {{ $t('tool.devTools.tools.codefmt.desc') }}</span>
        </h3>
        <div class="tool-actions">
      ...
      <div v-if="activeTool === 'cssgradient'" class="tool-panel">
        <h3 class="tool-title">
          {{ $t('tool.devTools.tools.cssgradient.name')
          }}<span class="tool-desc"> — {{ $t('tool.devTools.tools.cssgradient.desc') }}</span>
        </h3>
      ...
      <div v-if="activeTool === 'ymlprops'" class="tool-panel">
        <h3 class="tool-title">
          {{ $t('tool.devTools.tools.ymlprops.name')
          }}<span class="tool-desc"> — {{ $t('tool.devTools.tools.ymlprops.desc') }}</span>
        </h3>
        <div class="tool-actions">
      ...
      <div v-if="activeTool === 'colortool'" class="tool-panel">
        <h3 class="tool-title">
          {{ $t('tool.devTools.tools.colortool.name')
          }}<span class="tool-desc"> — {{ $t('tool.devTools.tools.colortool.desc') }}</span>
        </h3>
        <div class="color-row">
      ...
      <div v-if="activeTool === 'imgcompress'" class="tool-panel">
        <h3 class="tool-title">
          {{ $t('tool.devTools.tools.imgcompress.name')
          }}<span class="tool-desc"> — {{ $t('tool.devTools.tools.imgcompress.desc') }}</span>
        </h3>
        ...
        <template v-else>
          <div class="img-compress-config">
            ...
            <div class="img-config-row">
              <span>最大宽度：</span>
              <el-input-number v-model="imgCompressMaxWidth" :min="100" :max="8000" :step="100" style="width: 140px" />
              px
              <span style="margin-left: 16px">最大高度：</span>
              <el-input-number v-model="imgCompressMaxHeight" :min="100" :max="8000" :step="100" style="width: 140px" />
              px
            </div>
            <div class="tool-actions">
              <el-button type="primary" @click="compressImage">压缩</el-button>
              <el-button @click="resetImgCompress">重新上传</el-button>
              <el-button v-if="imgCompressedSrc" type="success" @click="downloadImg(imgCompressedSrc, 'compressed')"
                >下载压缩图</el-button
              >
            </div>
          </div>
          <div class="img-compare">
            <div class="img-compare-item">
              <div class="img-compare-title">
                原图<span class="img-size-tag">{{ imgCompressOrigSize }}</span>
              </div>
              <img :src="imgCompressSrc" class="img-compare-preview" />
            </div>
            <div class="img-compare-item" v-if="imgCompressedSrc">
              <div class="img-compare-title">
                压缩后<span class="img-size-tag img-size-green">{{ imgCompressNewSize }}</span>
              </div>
      ...
      <div v-if="activeTool === 'imgwatermark'" class="tool-panel">
        <h3 class="tool-title">
          {{ $t('tool.devTools.tools.imgwatermark.name')
          }}<span class="tool-desc"> — {{ $t('tool.devTools.tools.imgwatermark.desc') }}</span>
        </h3>
      ...
      <div v-if="activeTool === 'imgtoico'" class="tool-panel">
        <h3 class="tool-title">
          {{ $t('tool.devTools.tools.imgtoico.name')
          }}<span class="tool-desc"> — {{ $t('tool.devTools.tools.imgtoico.desc') }}</span>
        </h3>
          ...
          <div class="ico-config">
            <div class="img-config-row">
              ...
              <el-checkbox-group v-model="icoSizes">
                <el-checkbox v-for="s in [16, 24, 32, 48, 64, 128, 256]" :key="s" :value="s"
                  >{{ s }}×{{ s }}</el-checkbox
                >
              </el-checkbox-group>
            </div>
      ...
      <div v-if="activeTool === 'base64img'" class="tool-panel">
        <h3 class="tool-title">
          {{ $t('tool.devTools.tools.base64img.name')
          }}<span class="tool-desc"> — {{ $t('tool.devTools.tools.base64img.desc') }}</span>
        </h3>
        <div class="b64img-sections">
          <div class="b64img-section">
            ...
            <el-input
              v-model="b64ImgResult"
              type="textarea"
              :rows="4"
              readonly
              placeholder="上传图片后将显示Base64编码"
              style="font-family: monospace; font-size: 12px; margin-top: 8px"
            />
            <el-button v-if="b64ImgResult" size="small" style="margin-top: 6px" @click="copyOne(b64ImgResult)"
              >复制Base64</el-button
            >
          </div>
          <div class="b64img-section">
            ...
            <el-input
              v-model="b64DecodeInput"
              type="textarea"
              :rows="4"
              placeholder="粘贴Base64编码字符串（可含 data:image/...;base64, 前缀）"
            ...
            <div class="tool-actions" style="margin-top: 8px">
              ...
              <el-button
                ...
                size="small"
                type="success"
                @click="downloadImg(b64DecodedSrc, 'base64-decoded')"
                >下载图片</el-button
              >
            </div>
            <div v-if="b64DecodedSrc" style="margin-top: 10px">
              <img
                :src="b64DecodedSrc"
                style="max-width: 100%; max-height: 300px; border: 1px solid #e4e7ed; border-radius: 4px"
              />
            </div>
          </div>
        </div>
      ...
      <div v-if="activeTool === 'icondesign'" class="tool-panel">
        <h3 class="tool-title">
          {{ $t('tool.devTools.tools.icondesign.name')
          }}<span class="tool-desc"> — {{ $t('tool.devTools.tools.icondesign.desc') }}</span>
        </h3>
        <div class="icondesign-layout">
          <div class="icondesign-config">
            ...
            <div class="img-config-row">
              ...
              <el-input
                v-model="iconDesignBg"
                size="small"
                style="width: 100px; font-family: monospace"
                @input="renderIconDesign"
              />
            </div>
            <div class="img-config-row">
              <span>文字颜色：</span>
            ...
            <div class="img-config-row">
              <span>字号：</span>
              <el-input-number
                v-model="iconDesignFontSize"
                :min="20"
                :max="200"
                style="width: 120px"
                @change="renderIconDesign"
              />
              <span style="margin-left: 12px">大小：</span>
              <el-input-number
                v-model="iconDesignSize"
                :min="64"
                :max="1024"
                :step="64"
                style="width: 120px"
...
function toUnicode() {
  unicodeResult.value = unicodeText.value
    .split('')
    .map((c) => '\\u' + c.charCodeAt(0).toString(16).padStart(4, '0'))
    .join('')
}
function fromUnicode() {
  unicodeResult.value = unicodeText.value.replace(/\\u([0-9A-Fa-f]{4})/g, (_, hex) =>
...
const passDigit = ref(true)
const passSymbol = ref(true)
const passwords = ref([])
  ...
  },
  {
    ...
    items: [
      ...
      { label: '邮箱', pattern: '[\\w.+-]+@[\\w-]+(\\.[\\w-]+)+', text: '联系：admin@example.com 和 test@mail.cn' },
      {
        ...
        text: '张三110105199001011234 李五35010219851231002X'
      },
      { label: 'QQ号', pattern: '[1-9]\\d{4,10}', text: 'QQ:10001 客服:12345 群号:invalid99' },
      {
        label: 'IP地址',
        pattern: '\\b(?:(?:25[0-5]|2[0-4]\\d|[01]?\\d\\d?)\\.){3}(?:25[0-5]|2[0-4]\\d|[01]?\\d\\d?)\\b',
        text: '服务器:192.168.1.1 DNS:8.8.8.8 无效:999.1.1.1'
      }
    ]
  },
  {
    ...
    items: [
      ...
      { label: 'Hex颜色', pattern: '#[0-9A-Fa-f]{3,8}\\b', text: '主色#f59e0b 背景#fff 边框#e4e7ed 错误#F56C6C' },
      {
        label: 'URL链接',
        pattern: 'https?://[\\w./?=&#@%+~-]+',
        text: '访问 https://www.example.com/path?q=1 获取信息'
      }
    ]
  },
  {
    ...
    items: [
      { label: '完整日期', pattern: '\\d{4}[-/]\\d{2}[-/]\\d{2}', text: '出生2024-06-07 入职2025/01/15 2024-6-7' },
      {
        ...
        text: '创建于2024-06-07 14:30 更新2025/01/15 09:00'
      },
...
async function copyTextStats() {
  await navigator.clipboard.writeText(textStatsInput.value)
  ElMessage.success('已复制')
}
...
// JSON编辑器
const jsonEditInput = ref('')
const jsonEditValid = ref(null)
const jsonEditTree = ref('')
function jsonEditFormat() {
  try {
    const o = JSON.parse(jsonEditInput.value)
    jsonEditInput.value = JSON.stringify(o, null, 2)
    jsonEditValid.value = true
    renderJsonTree(o)
  } catch {
    jsonEditValid.value = false
  }
}
function jsonEditCompress() {
  try {
    const o = JSON.parse(jsonEditInput.value)
    jsonEditInput.value = JSON.stringify(o)
    jsonEditValid.value = true
  } catch {
    jsonEditValid.value = false
  }
...
function jsonEditValidate() {
  try {
    JSON.parse(jsonEditInput.value)
    jsonEditValid.value = true
...
function jsonEditOnInput() {
  try {
    const o = JSON.parse(jsonEditInput.value)
    jsonEditValid.value = true
    renderJsonTree(o)
  } catch {
    jsonEditValid.value = null
    jsonEditTree.value = ''
  }
}
function renderJsonTree(obj, depth = 0) {
  ...
  if (typeof obj !== 'object') {
    const c =
      typeof obj === 'string' ? COLORS.SUCCESS : typeof obj === 'number' ? COLORS.WARNING : COLORS.STATUS.WARNING
    return `<span style="color:${c}">${JSON.stringify(obj)}</span>`
  }
  if (Array.isArray(obj)) {
    let html = `<span style="color:${COLORS.TEXT_SECONDARY}">[</span><br>`
    obj.forEach((v, i) => {
      html += `${indent}&nbsp;&nbsp;<span style="color:${COLORS.TEXT_SECONDARY}">${i}:</span> ${renderJsonTree(v, depth + 1)}<br>`
    })
  ...
  Object.entries(obj).forEach(([k, v]) => {
    html += `${indent}&nbsp;&nbsp;<span style="color:${COLORS.TEXT_PRIMARY}">"${k}":</span> ${renderJsonTree(v, depth + 1)}<br>`
  })
  html += `${indent}<span style="color:${COLORS.TEXT_SECONDARY}">}</span>`
  return html
}
...
// JSON转换
const jsonConvertInput = ref('')
const jsonConvertTo = ref('xml')
const jsonConvertOutput = ref('')
function doJsonConvert() {
  try {
    const val = jsonConvertInput.value.trim()
    if (jsonConvertTo.value === 'xml') {
      const obj = JSON.parse(val)
      const toXml = (o, root = 'root') =>
        `<${root}>${
          typeof o === 'object'
            ? Object.entries(o)
                .map(([k, v]) => toXml(v, k))
      ...
      if (lines.length < 2) {
        ElMessage.error('CSV至少需要标题行和一行数据')
        return
      }
...
async function doCrypto() {
  ...
  try {
    if (cryptoAlgo.value === 'MD5') {
      ...
      cryptoResult.value = Array.from(new Uint8Array(hashBuffer))
...
function md5(string) {
  ...
  for (let k = 0; k < x.length; k += 16) {
    ...
    d = GG(d, a, b, c, x[k + 7], 9, 0xfcefa3f8)
    c = GG(c, d, a, b, x[k + 2], 14, 0x676f02d9)
    ...
    d = HH(d, a, b, c, x[k + 8], 11, 0xeaa127fa)
    ...
    d = II(d, a, b, c, x[k + 14], 10, 0xfe2ce6e0)
    c = II(c, d, a, b, x[k + 5], 15, 0xa3014314)
    b = II(b, c, d, a, x[k + 12], 21, 0x4e0811a1)
    a = II(a, b, c, d, x[k + 4], 6, 0xf7537e82)
    d = II(d, a, b, c, x[k + 9], 10, 0xbd3af235)
    c = II(c, d, a, b, x[k + 15], 15, 0x2ad7d2bb)
    b = II(b, c, d, a, x[k + 6], 21, 0xeb86d391)
    a = AddUnsigned(a, AA)
    b = AddUnsigned(b, BB)
    c = AddUnsigned(c, CC)
    d = AddUnsigned(d, DD)
...
function parseJwt() {
  ...
  if (parts.length !== 3) {
    jwtResult.value = { error: '无效的JWT格式（应为 header.payload.signature）' }
    return
  }
  try {
    const b64Decode = (str) => {
    ...
    jwtResult.value = { error: '解析失败: ' + e.message }
  }
}
...
async function sendHttp() {
  ...
  try {
    ...
    try {
      httpResponse.value = JSON.stringify(JSON.parse(text), null, 2)
    } catch (e) {
...
async function queryIp() {
  ...
  ipResult.value = null
  try {
    const url = ipQueryAddr.value.trim()
      ? `https://ipapi.co/${ipQueryAddr.value.trim()}/json/`
      : 'https://ipapi.co/json/'
...
// ===== HTML ↔ Markdown 互转 =====
const mdhtmlDir = ref('md2html')
const mdhtmlInput = ref('')
const mdhtmlOutput = ref('')
...
const tzSourceZone = ref('Asia/Shanghai')
const tzTargetZone = ref('America/New_York')
const tzResult = ref('')
const timezoneList = [
  'Asia/Shanghai',
  'Asia/Tokyo',
...
// ===== 代码格式化 =====
const codeFmtLang = ref('js')
const codeFmtInput = ref('')
const codeFmtOutput = ref('')
function formatCode() {
  ...
  try {
    ...
    codeFmtOutput.value = '格式化失败: ' + e.message
  }
}
...
function formatSQL(code) {
  ...
  result = result.replace(
    /\b(SELECT|FROM|WHERE|AND|OR|ORDER BY|GROUP BY|HAVING|LIMIT|INSERT INTO|VALUES|UPDATE|SET|DELETE FROM|CREATE TABLE|ALTER TABLE|LEFT JOIN|RIGHT JOIN|INNER JOIN|ON)\b/gi,
    '\n$1'
  )
  return result.replace(/^\n/, '').trim()
}
...
const gradColor1 = ref(COLORS.PRIMARY.toUpperCase())
const gradColor2 = ref(COLORS.SUCCESS.toUpperCase())
const gradCss = ref('')
      ...
      function parseYml(text, prefix = '') {
        ...
        while (i < arr.length) {
          ...
          if (!val) {
            ...
            while (
              j < arr.length &&
              (arr[j].trim() === '' || arr[j].match(/^\s{2,}/) || arr[j].startsWith('  ') || arr[j].startsWith('\t'))
            ) {
...
function onWmUpload(file) {
  ...
  const reader = new FileReader()
  reader.onload = (e) => {
    wmSrc.value = e.target.result
  }
  reader.readAsDataURL(file.raw)
}

function addWatermark() {
  if (!wmSrc.value || !wmText.value.trim()) {
    ElMessage.error('请上传图片并输入水印文字')
    return
  }
  const img = new Image()
  img.src = wmSrc.value
...
function onIcoUpload(file) {
  ...
  const reader = new FileReader()
  reader.onload = (e) => {
    icoSrc.value = e.target.result
  }
  reader.readAsDataURL(file.raw)
}

function convertToIco() {
  if (!icoSrc.value || !icoSizes.value.length) {
    ElMessage.error('请上传图片并选择尺寸')
    return
  }
  const img = new Image()
...
const emojiRecent = ref(
  (() => {
    const d = emojiStore.get()
    return Array.isArray(d) ? d.slice(0, 18) : []
  })()
)
...
.radix-item code {
  color: var(--text-primary, #303133);
  font-weight: 500;
  background: var(--el-border-color-extra-light, #e8eaed);
  padding: 2px 6px;
  border-radius: 3px;
  font-size: 13px;
}
...
html.dark .devtools-container [style*='color:#303133'] {
  color: var(--dark-text-primary, #e5eaf3) !important;
}
html.dark .devtools-container [style*='color:#606266'] {
  color: var(--dark-text-regular, #a3a6ad) !important;
}
html.dark .devtools-container [style*='color:#909399'] {
  color: var(--dark-text-secondary, #73767a) !important;
}
html.dark .devtools-container [style*='color:#c0c4cc'] {
  color: var(--dark-text-secondary, #73767a) !important;
}
...
html.dark .devtools-container [style*='background:#e8eaed'] {
  background-color: var(--dark-bg-hover, #262727) !important;
}
html.dark .devtools-container [style*='background:#fef0f0'] {
  background-color: #3a1a1a !important;
}
html.dark .devtools-container [style*='background:#f0f9eb'] {
  background-color: #1a2e1a !important;
}
html.dark .devtools-container [style*='border:1px solid #e4e7ed'] {
