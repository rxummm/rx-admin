    ...
    <div style="display: flex; gap: 12px; flex: 1; min-height: 0;">
        ...
        <div style="flex: 1; overflow-y: auto; border: 1px solid var(--border-light, #ebeef5); border-radius: 4px;">
          <div v-for="item in filteredEndpoints" :key="item.paths?.[0] + item.methods?.[0]"
            ...
            <div style="display: flex; align-items: center; gap: 6px;">
              ...
              <span style="font-size: 12px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">{{ item.paths?.[0] }}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style="flex: 1; display: flex; flex-direction: column; gap: 8px;">
          ...
          <el-tab-pane label="Query参数" name="params">
            <el-table :data="queryParams" border>
              ...
              <el-table-column label="Value">
                <template #default="{ row }"><el-input v-model="row.value" size="small" placeholder="参数值" /></template>
              </el-table-column>
              <el-table-column width="60">
                <template #default="{ $index }"><el-button link type="danger" @click="queryParams.splice($index, 1)">删除</el-button></template>
              </el-table-column>
            </el-table>
            <el-button link type="primary" @click="queryParams.push({key:'',value:''})" style="margin-top: 4px;">+ 添加参数</el-button>
          </el-tab-pane>
        </el-tabs>
        ...
        <div v-if="responseStatus !== null" style="background: #1e1e1e; border-radius: 4px; padding: 12px;">
          ...
          <pre style="color:#d4d4d4; font-size: 12px; margin: 0; white-space: pre-wrap; word-break: break-all; max-height: 200px; overflow-y: auto;">{{ responseBody }}</pre>
        </div>
      </div>
    </div>
  </div>
...
const endpoints = ref([]); const searchKeyword = ref('')
const selected = ref(null); const debugMethod = ref('GET'); const debugUrl = ref('')
const queryParams = ref([{ key: '', value: '' }])
const requesting = ref(false); const responseStatus = ref(null)
const responseTime = ref(0); const responseBody = ref('')
...
const doRequest = async () => {
  if (!debugUrl.value) { ElMessage.warning('请输入请求路径'); return }
  requesting.value = true; responseStatus.value = null
  ...
  try {
    ...
    switch (debugMethod.value) {
      case 'POST': res = await request.post(debugUrl.value, params); break
      case 'PUT': res = await request.put(debugUrl.value, params); break
      case 'DELETE': res = await request.delete(debugUrl.value, { params }); break
      default: res = await request.get(debugUrl.value, { params }); break
    }
...
onMounted(async () => {
  try { const res = await getEndpointsApi(); endpoints.value = res.data || [] } catch (e) { /* */ }
})
</script>
