    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="700px" :close-on-click-modal="false">
      <el-form ref="formRef" :model="form" :rules="formRules" label-width="90px">
        ...
        <el-form-item label="场景/回目">
          <el-input v-model="form.relatedScene" placeholder="如：第五回 游幻境指迷十二钗" />
        </el-form-item>
        <el-form-item label="相关人物">
          <el-input v-model="form.relatedCharacter" placeholder="如：贾宝玉、林黛玉" />
        </el-form-item>
        ...
        <el-form-item label="诗词赏析">
          <el-input v-model="form.appreciation" type="textarea" :rows="6" placeholder="请输入赏析内容" />
        </el-form-item>
      </el-form>
      <template #footer>
...
<script setup>
defineOptions({ name: 'ClassicsHonglouPoems' })
...
async function fetchData() {
  ...
  try {
    ...
    const res = await getHonglouPoemPageApi(params)
...
async function handleDelete(row) {
  try {
    ...
    await deleteHonglouPoemApi(row.id)
...
async function handleBatchDelete() {
  ...
  try {
    ...
    await batchDeleteHonglouPoemApi(selectedIds.value)
...
async function handleSubmit() {
  ...
  try {
    if (isEdit.value) {
      await updateHonglouPoemApi({ ...form })
      ElMessage.success('修改成功')
    } else {
      await addHonglouPoemApi({ ...form })
