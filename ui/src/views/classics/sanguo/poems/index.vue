<template>
  <div class="page-container">
    <div class="search-bar">
      ...
      <el-select v-model="filterCategory" placeholder="分类筛选" clearable style="width: 130px" @change="fetchData">
      ...
      <div style="flex: 1" />
      <el-button type="primary" @click="handleAdd" v-if="userStore.hasPerm('classics:sanguo:poem:add')">
        <el-icon><Plus /></el-icon> 新增诗词
      </el-button>
      <el-button type="danger" @click="handleBatchDelete" v-if="userStore.hasPerm('classics:sanguo:poem:delete')" :disabled="selectedIds.length === 0">
    ...
    <div class="classics-table-wrapper">
      <el-table :data="sortedTableData" border stripe v-loading="loading" :max-height="tableMaxHeight" style="width: 100%"
        ...
        <el-table-column v-if="visibleColumns.includes('dynasty')" prop="dynasty" label="朝代" width="90" />
        <el-table-column v-if="visibleColumns.includes('category')" prop="category" label="分类" width="100">
          <template #default="{ row }">
            <el-tag v-if="row.category" size="small">{{ row.category }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column v-if="visibleColumns.includes('chapter')" prop="chapter" label="章节" width="120" show-overflow-tooltip sortable />
        ...
        <el-table-column label="操作" width="180" fixed="right">
          <template #default="{ row }">
            ...
            <el-button link type="primary" size="small" @click="handleEdit(row)" v-if="userStore.hasPerm('classics:sanguo:poem:edit')">编辑</el-button>
            <el-button link type="danger" size="small" @click="handleDelete(row)" v-if="userStore.hasPerm('classics:sanguo:poem:delete')">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="700px" :close-on-click-modal="false">
      <el-form ref="formRef" :model="form" :rules="formRules" label-width="90px">
        ...
        <el-form-item label="朝代">
          <el-input v-model="form.dynasty" placeholder="如：东汉、西晋" />
        </el-form-item>
        <el-form-item label="分类">
          <el-input v-model="form.category" placeholder="如：赞诗、叹词、咏史诗" />
        </el-form-item>
        <el-form-item label="章节">
          <el-input v-model="form.chapter" placeholder="如：第五回" />
        </el-form-item>
        <el-form-item label="相关人物">
          <el-input v-model="form.relatedCharacter" placeholder="如：曹操、诸葛亮" />
        </el-form-item>
        <el-form-item label="诗词内容" prop="content">
          <el-input v-model="form.content" type="textarea" :rows="10" placeholder="请输入诗词正文" />
        </el-form-item>
        <el-form-item label="翻译/注释">
          <el-input v-model="form.translation" type="textarea" :rows="4" placeholder="请输入翻译或注释" />
        </el-form-item>
    ...
    <el-dialog v-model="viewVisible" title="诗词详情" width="700px">
      <div v-if="viewData" class="poem-detail">
        ...
        <div class="poem-meta">
          ...
          <span v-if="viewData.dynasty">{{ viewData.dynasty }}</span>
          <el-tag v-if="viewData.category" size="small">{{ viewData.category }}</el-tag>
          <span v-if="viewData.chapter">{{ viewData.chapter }}</span>
          <span v-if="viewData.relatedCharacter">相关人物：{{ viewData.relatedCharacter }}</span>
        </div>
        <div class="poem-content">
          <pre>{{ viewData.content }}</pre>
        </div>
        <div v-if="viewData.translation" class="poem-extra">
          <el-divider content-position="left">翻译/注释</el-divider>
...
<script setup>
defineOptions({ name: 'ClassicsSanguoPoems' })
import { ref, reactive, computed, onMounted, onUnmounted } from 'vue'
import { useTableHeight } from '@/composables/useTableHeight'
import { ElMessage, ElMessageBox } from 'element-plus'
import { useUserStore } from '@/stores/user'
import {
  getSanguoPoemPageApi,
  getSanguoPoemDetailApi,
  addSanguoPoemApi,
  updateSanguoPoemApi,
  deleteSanguoPoemApi,
  batchDeleteSanguoPoemApi
} from '@/api/sanguo'
...
async function fetchData() {
  ...
  try {
    ...
    const res = await getSanguoPoemPageApi(params)
...
async function handleDelete(row) {
  try {
    ...
    await deleteSanguoPoemApi(row.id)
...
async function handleBatchDelete() {
  ...
  try {
    ...
    await batchDeleteSanguoPoemApi(selectedIds.value)
...
async function handleSubmit() {
  ...
  try {
    if (isEdit.value) {
      await updateSanguoPoemApi({ ...form })
      ElMessage.success('修改成功')
    } else {
      await addSanguoPoemApi({ ...form })
