<template>
  <div class="page-container">
    ...
    <div class="export-log-table-wrapper">
      <el-table :data="tableData" :max-height="tableMaxHeight" v-loading="loading" stripe border style="width: 100%">
        <el-table-column prop="id" label="ID" width="70" />
        <el-table-column prop="username" label="用户名" width="120" />
        <el-table-column prop="exportType" label="导出类型" width="100">
          <template #default="{ row }">
            <el-tag :type="row.exportType === 'excel' ? 'success' : 'warning'" size="small">{{
              row.exportType?.toUpperCase()
            }}</el-tag>
          </template>
        </el-table-column>
...
const { tableMaxHeight, calcTableMaxHeight } = useTableHeight('.export-log-table-wrapper')
...
function buildParams() {
  const p = {
    ...
    exportType: searchForm.exportType
  }
  if (searchForm.timeRange && searchForm.timeRange.length === 2) {
    p.startTime = searchForm.timeRange[0]
    p.endTime = searchForm.timeRange[1]
  }
  return p
}
...
async function fetchData() {
  loading.value = true
  try {
    const res = await getExportLogPageApi(buildParams())
...
<style scoped>
.export-log-table-wrapper {
  flex: 1;
  display: flex;
  flex-direction: column;
  min-height: 0;
}
.export-log-table-wrapper :deep(.page-pagination) {
  margin-top: 12px;
  flex-shrink: 0;
}
</style>
