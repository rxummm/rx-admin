  ...
  <div class="page-container">
    ...
    <div class="job-log-table-wrapper">
      <el-table
        ...
        @selection-change="(v) => (selectedIds = v.map((r) => r.id))"
        v-loading="loading"
        stripe
        border
        style="width: 100%"
      >
        ...
        <el-table-column prop="durationMs" label="耗时(ms)" width="100">
          <template #default="{ row }"
            ><span :style="{ color: row.durationMs > 5000 ? '#f56c6c' : '' }">{{ row.durationMs }}ms</span></template
          >
        </el-table-column>
        ...
        <el-table-column label="操作" width="70" fixed="right">
          <template #default="{ row }"
            ><el-button size="small" link type="danger" @click="handleDelete(row.id)">删除</el-button></template
          >
        </el-table-column>
      </el-table>
...
async function fetchData() {
  loading.value = true
  try {
    const res = await getJobLogPageApi(buildParams())
...
async function handleDelete(id) {
  ...
  await deleteJobLogApi(id)
  ElMessage.success('删除成功')
  fetchData()
}
async function handleBatchDelete() {
  await ElMessageBox.confirm(`确认删除${selectedIds.value.length}条？`, '提示', { type: 'warning' })
  await deleteJobLogBatchApi(selectedIds.value)
  ElMessage.success('删除成功')
  fetchData()
}
onMounted(() => {
