<template>
  <div class="page-container">
    <div class="search-bar">
      <el-input
        v-model="keyword"
        :placeholder="$t('monitor.slowQuery.sql')"
        clearable
        style="width: 240px"
    ...
    <div class="virtual-table-wrapper">
      ...
      <RecycleScroller class="vscroller" :items="tableData" :item-size="44" key-field="id" v-slot="{ item }">
        <div class="vrow" :class="{ even: tableData.indexOf(item) % 2 === 1 }">
          <div class="vc vc-chk">
            <el-checkbox :model-value="selectedIds.includes(item.id)" @change="toggleSelect(item)" />
          </div>
          <div class="vc vc-id">{{ item.id }}</div>
          <div class="vc vc-sql" :title="item.sqlText">{{ item.sqlText }}</div>
          <div class="vc vc-cost">
            <el-tag
              :type="item.costTimeMs > 5000 ? 'danger' : item.costTimeMs > 3000 ? 'warning' : 'info'"
              size="small"
              >{{ item.costTimeMs }}</el-tag
            >
          </div>
            ...
            <el-tag
              :type="
                item.queryType === 'SELECT'
                  ? 'primary'
                  : item.queryType === 'INSERT'
                    ? 'success'
                    : item.queryType === 'UPDATE'
                      ? 'warning'
                      : 'danger'
              "
              size="small"
              >{{ item.queryType }}</el-tag
            >
          </div>
...
function handleDelete(row) {
  ElMessageBox.confirm(t('common.deleteConfirm'), t('common.tip'), { type: 'warning' })
    .then(async () => {
...
function handleBatchDelete() {
  ...
  ElMessageBox.confirm(t('common.confirmBatchDelete', { count: selectedIds.value.length }), t('common.tip'), {
    type: 'warning'
  })
    .then(async () => {
...
function handleClearAll() {
  ElMessageBox.confirm(t('common.clear') + '?', t('common.tip'), { type: 'warning' })
    .then(async () => {
