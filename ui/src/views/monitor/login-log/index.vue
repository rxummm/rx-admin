  ...
  <div class="page-container">
    <div class="search-bar">
      <el-input
        ...
        :placeholder="$t('monitor.loginLog.username')"
        clearable
        style="width: 160px"
      />
      <el-select
        v-model="searchForm.status"
        :placeholder="$t('monitor.loginLog.status')"
        clearable
      ...
      <el-date-picker
        ...
        type="datetimerange"
        :range-separator="$t('monitor.loginLog.to')"
        :start-placeholder="$t('monitor.loginLog.start')"
        :end-placeholder="$t('monitor.loginLog.end')"
          ...
          <el-table-column prop="status" :label="$t('monitor.loginLog.status')" width="80">
            <template #default="{ row }">
              <el-tag :type="row.status === 1 ? 'success' : 'danger'" size="small">{{
                row.status === 1 ? $t('monitor.loginLog.success') : $t('monitor.loginLog.fail')
              }}</el-tag>
            </template>
          </el-table-column>
          <el-table-column
            prop="failReason"
            :label="$t('monitor.loginLog.failReason')"
            min-width="160"
            show-overflow-tooltip
          />
...
function handleSelectionChange(vals) {
  selectedIds.value = vals.map((r) => r.id)
}
...
async function handleBatchDelete() {
  await ElMessageBox.confirm(t('common.confirmBatchDelete', { count: selectedIds.value.length }), t('common.tip'), {
    type: 'warning'
  })
  await deleteLoginLogBatchApi(selectedIds.value)
