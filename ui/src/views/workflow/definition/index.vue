  ...
  <div class="page-container">
    ...
    <div class="table-container">
      <el-table :data="tableData" v-loading="loading" border stripe>
        <el-table-column prop="name" :label="$t('workflow.definition.name')" />
        <el-table-column prop="code" :label="$t('workflow.definition.code')" />
        <el-table-column prop="category" :label="$t('workflow.definition.category')" />
    ...
    <el-dialog v-model="dialogVisible" :title="isEdit ? $t('common.edit') : $t('common.add')" width="600px">
      <el-form :model="form" label-width="100px">
        <el-form-item :label="$t('workflow.definition.name')"><el-input v-model="form.name" /></el-form-item>
        <el-form-item :label="$t('workflow.definition.code')"><el-input v-model="form.code" :disabled="isEdit" /></el-form-item>
        <el-form-item :label="$t('workflow.definition.category')"><el-input v-model="form.category" /></el-form-item>
        <el-form-item :label="$t('workflow.definition.description')"><el-input v-model="form.description" type="textarea" /></el-form-item>
      </el-form>
      <template #footer>
...
const loadData = async () => {
  loading.value = true
  try {
    const { data } = await request({ url: '/wf/definition/page', method: 'get', params: { keyword: keyword.value, status: statusFilter.value, page: page.value, size: size.value } })
...
const handleSubmit = async () => {
  if (isEdit.value) {
    await request({ url: '/wf/definition', method: 'put', data: form.value })
  } else {
    await request({ url: '/wf/definition', method: 'post', data: form.value })
  }
...
const handleDelete = async (id) => {
  await request({ url: `/wf/definition/${id}`, method: 'delete' })
  ElMessage.success(t('common.success'))
  loadData()
}
