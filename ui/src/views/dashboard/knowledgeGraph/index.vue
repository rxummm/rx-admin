<template>
  <div class="kg-page" v-loading="loading">
    ...
    <el-card shadow="hover" class="section-card" v-if="tourSteps.length">
      <template #header>
        <div class="section-header">
          <el-icon :size="20" color="#409eff"><Compass /></el-icon>
          <span>代码导览</span>
        </div>
      </template>
      <el-timeline>
...
// Colors
const colorPalette = ['#409eff', '#67c23a', '#e6a23c', '#f56c6c', '#8b5cf6', '#909399', '#38bdf8', '#ec4899', '#14b8a6', '#f97316']
const timelineColors = ['#409eff', '#67c23a', '#e6a23c', '#f56c6c', '#8b5cf6']
...
function renderLayerChart() {
  ...
  if (!layerChart) layerChart = echarts.init(layerChartRef.value)
...
function renderEdgeChart() {
  ...
  if (!edgeChart) edgeChart = echarts.init(edgeChartRef.value)
  edgeChart.setOption({
    tooltip: { trigger: 'axis', axisPointer: { type: 'shadow' } },
    grid: { left: '3%', right: '6%', top: 10, bottom: 5, containLabel: true },
