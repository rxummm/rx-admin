    ...
    <el-tabs v-model="activeTab">
      <el-tab-pane label="消息模板" name="templates">
        ...
        <div class="pagination-wrap">
          <el-pagination
            v-model:current-page="tPage.page"
            v-model:page-size="tPage.size"
            :page-sizes="[10, 20, 50]"
            :total="tPage.total"
      ...
      <el-tab-pane label="发送记录" name="records">
        ...
        <el-table
          :data="records"
          v-loading="rLoading"
          @selection-change="(v) => (rSelected = v.map((r) => r.id))"
          stripe
          border
        >
          ...
          <el-table-column label="操作" width="120" fixed="right">
            <template #default="{ row }">
              <el-button v-if="row.status === 2" size="small" link type="warning" @click="retry(row.id)"
                >重发</el-button
              >
        ...
        <div class="pagination-wrap">
          <el-pagination
            v-model:current-page="rPage.page"
            v-model:page-size="rPage.size"
            :page-sizes="[10, 20, 50]"
            :total="rPage.total"
            layout="total,sizes,prev,pager,next"
            @change="fetchRecords"
    ...
    <el-dialog v-model="tDialogVisible" :title="editingTemplate?.id ? '编辑模板' : '新增模板'" width="600px">
      <el-form :model="tForm" label-width="80px">
        ...
        <el-form-item label="标题模板"
          ><el-input v-model="tForm.titleTemplate" placeholder="您好{username}，..."
        /></el-form-item>
        <el-form-item label="内容模板"
          ><el-input v-model="tForm.contentTemplate" type="textarea" :rows="4"
        /></el-form-item>
        <el-form-item label="通道"><el-input v-model="tForm.channels" placeholder="message,email" /></el-form-item>
        <el-form-item label="状态"
          ><el-switch v-model="tForm.status" :active-value="1" :inactive-value="0"
        /></el-form-item>
      </el-form>
      <template #footer>
...
import {
  ...
  updateTemplateApi,
  deleteTemplateApi,
  getNotifyRecordPageApi,
  deleteNotifyRecordApi,
  deleteNotifyRecordsBatchApi,
  retryNotifyApi
} from '@/api/notifyCenter'
...
const tSearch = reactive({ name: '' })
const tPage = reactive({ page: 1, size: 10, total: 0 })
...
async function fetchTemplates() {
  tLoading.value = true
  try {
    const res = await getTemplatePageApi({ page: tPage.page, size: tPage.size, name: tSearch.name || undefined })
    templates.value = res.data.records || []
    tPage.total = res.data.total || 0
  } catch {
...
async function saveTemplate() {
  if (editingTemplate.value?.id) {
    tForm.id = editingTemplate.value.id
    await updateTemplateApi(tForm)
  } else await addTemplateApi(tForm)
  ElMessage.success('保存成功')
  tDialogVisible.value = false
  fetchTemplates()
}
...
const rSelected = ref([])
const rSearch = reactive({ channel: null, status: null })
const rPage = reactive({ page: 1, size: 10, total: 0 })
async function fetchRecords() {
  rLoading.value = true
  try {
    const res = await getNotifyRecordPageApi({
      page: rPage.page,
      size: rPage.size,
      channel: rSearch.channel,
      status: rSearch.status
    })
    records.value = res.data.records || []
    rPage.total = res.data.total || 0
  } catch {
    ElMessage.error('加载失败')
  } finally {
...
async function deleteRecord(id) {
  await ElMessageBox.confirm('确认删除？', '提示', { type: 'warning' })
  await deleteNotifyRecordApi(id)
  ElMessage.success('删除成功')
  fetchRecords()
}
async function batchDeleteRecords() {
  await ElMessageBox.confirm(`确认删除${rSelected.value.length}条？`, '提示', { type: 'warning' })
  await deleteNotifyRecordsBatchApi(rSelected.value)
  ElMessage.success('删除成功')
  fetchRecords()
}
async function retry(id) {
  await retryNotifyApi(id)
