...
CREATE TABLE audio_transcription (
    ...
    language VARCHAR(20) DEFAULT 'zh' COMMENT '语言代码',
    full_text TEXT COMMENT '完整转写文本',
    duration DOUBLE COMMENT '音频时长(秒)',
...
CREATE TABLE audio_segment (
    ...
    start_time DOUBLE NOT NULL COMMENT '开始时间(秒)',
    end_time DOUBLE NOT NULL COMMENT '结束时间(秒)',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='音频分段表';

-- 添加外键约束（主表创建完成后）
ALTER TABLE audio_segment
    ADD CONSTRAINT fk_audio_segment_transcription
...
-- 列表查询权限
INSERT INTO sys_menu (menu_name, parent_id, menu_type, path, component, perms, icon, sort, status, visible)
SELECT '列表查询', @audio_menu_id, 3, '', '', 'audio:transcription:list', '', 4, 1, 1
